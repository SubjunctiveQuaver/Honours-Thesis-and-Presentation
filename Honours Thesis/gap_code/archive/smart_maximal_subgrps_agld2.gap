...
getStuff2 := function( d )
  local AffS, G, n, M, noBase, res, nr, max;
  ...
  for M in max do 
    if IsPrimitive( M ) then
      nr := nr + 1;
      if Size( BaseOfGroup( M ) ) > d then Add( res, M ); fi; # use GAP base command
      # if Length( GreedyBase( M ) ) > d then Add( res, M ); fi; # use greedy base alg
    fi;
  od;
