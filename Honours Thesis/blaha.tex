...
\added{3}{
    \begin{example}\label{eg:blaha_elem_technical_construction_example}
        In the above construction, set $n = 3$ and $X = [3]$. We use multiplicative notation for $(\Z/2\Z)^3$.
        \begin{itemize}
            ...
            \item The action $\mathcal{R}_Y$ is defined as such: for $\alpha \in G(Y)$ and say $\sigma = \sigma_1\sigma_2\sigma_3 = \sigma_2\sigma_1\sigma_3 \in G(X)$, we have
                  ...
                  It can be seen that $\mathcal{R}_Y$ acts regularly (like $\mathcal{R}$) for $G(Y)$, and trivially for $G(X \setminus Y) = \langle\sigma_2\rangle = \{1,\sigma_2\}$.
        ...
        As an extension, let us consider $Y = \{1,3\} \in \mathcal{C}$ and $\alpha \in G(Y)$. Then with $Z = \{1,2\}$ let us consider the stabiliser
        ...
        If $W = Y \cap Z = \{1\}$, then $G(W) = \{1,\sigma_1\}$ and $G(Y)$ has TODO
    \end{example}}
...
In the subsequent lemmas, we use the restricted action of the permutation group $G_\mathcal{C}(Z)$ on the block $G(Y) \subseteq \Omega_\mathcal{C}$ for $Z \subseteq X$ as described above. \added{3}{The following generalises the final observation in \autoref{eg:blaha_elem_technical_construction_example}.}
