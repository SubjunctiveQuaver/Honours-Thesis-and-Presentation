...
\added{4}{Another important matrix group that we briefly discuss is the symplectic group. The following may be found in section 7.7 of \cite{dixon_mortimer_perm_groups1996}. Let $K$ be a field and consider $K^d$, where $d = 2m$ is even. Let
    ...
    \begin{definition}\label{def:symplectic_group}
        For $d \geq 2$ even, the \textbf{symplectic group} is $\Sp_d(K) = \{a \in \GL_d(K) : afa^\top = f\} \leq \GL_d(K)$, which acts on $K^d$ by matrix multiplication: $u^a := ua$ for $a \in \Sp_d(K)$ and $u \in K^d$ (which we treat as row vectors).
    \end{definition}
    ...
    where $d = 2m$ (there is a typo on page 245 on \cite{dixon_mortimer_perm_groups1996}). Moreover, the quotient of $\Sp_d(q)$ by its centre gives a finite simple group of Lie type, as described in the classification theorem (\autoref{thm:cfsg}). We are primarily interested in the case that $K = \F_2$ for the next chapter, where $f = e + e^\top$.

    \begin{definition}\label{def:orthogonal_group}
        For any $d \geq 1$, the \textbf{(general) orthogonal group} is $\GO_d(K) = \{a \in \GL_d(K) : a^\top a = 1\} \leq \GL_d(K)$.
    \end{definition}

    For $K = \F_q$, again write $\GO_d(q)$ instead of $\GO_d(\F_q)$. In the case that $d = 2m$ is even and $q = 2$, it can be shown that
    $$|\GO_{d+1}(2)| = 2^{m^2}(2^2 - 1)(2^4 - 1)\dotsb(2^d - 1).$$
    Note that this is equal to $|\Sp_d(2)|$. This is no coincidence; in the special case of $q = 2$, symplectic groups are related to orthogonal groups. A proof of the next theorem can be found in \cite{} TODO.

    \begin{theorem}
        For even $d$, the symplectic group $\Sp_d(2)$ is isomorphic to the orthogonal group $\GO_{d+1}(2)$.
    \end{theorem}
