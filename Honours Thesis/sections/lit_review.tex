...
\begin{theorem}\label{thm:new_result}
    Let $G$ be a primitive subgroup of $\AGL_d(2)$ for some $d$, with degree $n = 2^d$.
    \begin{enumerate}[(i)]
...
\begin{proof}[Proof of main theorem]
    ...
    Note that these observations can be adapted for any group $G$, to find (primitive) subgroups $H$ of $G$ with $b(H) \geq r$ for any given $r$, by simply replacing ``$d + 1$'' with ``$r$''. If we wish to ignore the condition of primitivity, we may simply ignore observation 2 above.
