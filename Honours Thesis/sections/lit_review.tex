...
\begin{theorem}\label{thm:new_result}
    \begin{enumerate}[(i)]
        \item For odd $d \leq 9$ and $d = 2$, the only primitive group $G \leq \Sym(n)$ satisfying $b(G) = \log n + 1$ is (permutation isomorphic to) the affine group $\AGL_d(2)$.
        \item For even $4 \leq d \leq 10$, the only primitive groups $G \leq \Sym(n)$ satisfying $b(G) = \log n + 1$ are $\AGL_d(2)$ and $2^d : \Sp_d(2)$.
    \end{enumerate}
\end{theorem}

Based on this behaviour, we make the following conjecture in the direction of \autoref{qn:moscatiello_roney-dougal2021}:
...
\begin{conjecture}\label{conj:new_result}
    ...
    \begin{enumerate}[(i)]
        \item $n = 2^d$ and $G$ is the affine group $\AGL_d(2)$; or
        \item $n = 2^d$ with $d \geq 4$ even, and $G$ is the split extension $2^d : \Sp_d(2)$.
    \end{enumerate}
\end{conjecture}

The remainder of this section is dedicated to our approach to proving \autoref{thm:new_result}.
