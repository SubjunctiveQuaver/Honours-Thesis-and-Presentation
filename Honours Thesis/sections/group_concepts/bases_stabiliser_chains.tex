...
\begin{example}\label{eg:rubiks_group_random}
    ...
    \end{multline*}
    represents the following valid restickering of the Rubik's cube.
    ...
    \end{multline*}
    that is, apply this move sequence to the solved state to get the state $x$ in the above diagram. See the \hyperref[app:rubiks_group]{appendix} for relevant \texttt{GAP} code (that also allows us to check if arbitrary restickerings are valid, and for those that are, find a sequence of moves to solve it, all using group theory).
\end{example}
