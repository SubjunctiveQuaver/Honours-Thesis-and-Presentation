...
\begin{example}[Rubik's group]\label{eg:rubiks_group}
    ...
    To mimic move sequences on the cube, we define $G = \langle U,L,F,R,B,D \rangle \leq \Sym(48)$ where
    \begin{itemize}
    ...
    \end{itemize}
    Then for $g \in G$, applying the move sequence $g$ moves sticker $i$ to the position of sticker $i^g$; this is the natural action. See \hyperref[app:rubiks_group]{appendix} for \texttt{GAP} code related to implementation. Note that each generator of $G$ above has order 4 (recall that the order of a permutation is the lcm of the cycle lengths), and by computing
    ...
    we see that the commutator $RUR^{-1}U^{-1}$ and the move sequence $RU$ have orders $\lcm(6,3,6,3) = 6$ and $\lcm(15,7,3,7) = 105$ respectively; this is the smallest number of times these move sequences must be applied to return to the initial state.
