...
\added{2}{There is a similar notion of isomorphic permutation groups, where the natural action is preserved:
    ...
    The pair $(\tau,\psi)$ is a \textbf{permutation isomorphism} from $G$ to $H$; then $(\tau^{-1},\psi^{-1})$ is a permutation isomorphism from $H$ to $G$ (so that permutation isomorphism is an equivalence relation). The condition on $\tau,\psi$ says that the following diagram commutes \added{4}{for all $g \in G$}:}
...
\begin{example}\label{eg:action_D8_on_square}
    ...
    \begin{center}
        \begin{tikzpicture}[x=1cm,y=1cm]
            ...
            \Edge(l2)(l3) % s
            \Edge[color=magenta](l4)(l5) % sr
            \tikzset{EdgeStyle/.style={-}}
            {\Vertex[L={v_1},Lpos=180]{1} \SO[L={v_2},Lpos=180](1){2} \EA[L={v_3}](2){3} \NO[L={v_4}](3){4}
            ...
            \Edge(l2)(l3) % sr^2
            \tikzset{EdgeStyle/.style={-}}
            \Vertex[x=4,y=0,L={v_1},Lpos=180]{1} \SO[L={v_2},Lpos=180](1){2} \EA[L={v_3}](2){3} \NO[L={v_4}](3){4}
            ...
            % BOTTOM LEFT
            \tikzset{EdgeStyle/.style={-}}
            \Vertex[x=0,y=-3,L={v_1},Lpos=180]{1} \SO[L={v_2},Lpos=180](1){2} \EA[L={v_3}](2){3} \NO[L={v_4}](3){4}
            \AddVertexColor{yellow}{1}
            ...
            % BOTTOM RIGHT
            \tikzset{EdgeStyle/.style={-}}
            \Vertex[x=4,y=-3,L={v_1},Lpos=180]{1} \SO[L={v_2},Lpos=180](1){2} \EA[L={v_3}](2){3} \NO[L={v_4}](3){4}
            \AddVertexColor{red}{1}
            ...
            \Edge[label=$sr^2$](a1)(a2)
            \tkzDefPoint(1.5,-1.5){a1}
            \tkzDefPoint(3.5,-2.5){a2}
            \Edge[label=$sr$,labeltext=magenta,color=magenta](a1)(a2)
        \end{tikzpicture}
    \end{center}

    \added{1}{By renaming elements of $V$, we see that this action is equivalent} to the action of $D_8$ on the set $\{1,2,3,4\}$ defined by $r \mapsto (1,2,3,4)$ and $s \mapsto (1,4)(2,3)$, so we see that \added{1}{$D_8$ is isomorphic to a subgroup of $\Sym(4)$}.
