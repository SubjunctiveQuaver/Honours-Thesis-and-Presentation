...
\begin{example}[Rubik's group]\label{eg:rubiks_group}
    ...
    To mimic moves on the cube, we define $G = \langle U,L,F,R,B,D \rangle \leq \Sym(48)$ where
    \begin{itemize}
    ...
    See \hyperref[app:rubiks_group]{appendix} for \texttt{GAP} code related to implementation. The following analysis on $G$ is adapted from \cite{schonert_GAP}. Firstly, \texttt{GAP} computes that $|G| = 43\,252\,003\,274\,489\,856\,000 \approx 4.3 \cdot 10^{19}$ Now, the Rubik's group has two orbits under its natural action:
