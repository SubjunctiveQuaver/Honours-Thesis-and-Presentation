...
Another useful property of actions is that if $\alpha^g = \beta$ for $g \in G$ and $\alpha,\beta \in \Omega$, then $\beta^{g^{-1}} = \alpha$; this follows from the fact that $\varphi(g^{-1}) = \varphi(g)^{-1}$. We now provide some examples of actions on groups.
...
\added{1}{\begin{example}[representations]\label{eg:representations}
        Let $G$ be a group and $\Omega = V$, a $K$-vector space. Let $\rho : G \to \Sym(V)$ be a homomorphism with $\rho[G] \leq \GL(V) = \Aut(V)$, the invertible linear maps $V \to V$. Then $\rho$ is an action \added{2}{of $G$ on $V$}, but we typically write $\rho : G \to \GL(V)$ and call it a \textbf{representation} of $G$; these are studied in a branch of mathematics called representation theory, which \added{2}{analyses $G$ using} linear actions on a vector space, whose properties are well-known.
    \end{example}}
...
\added{2}{This notion of equivalence says that applying $\varphi$ then relabelling $\Omega$ via $\tau$ is the same as relabelling $\Omega$ via $\tau$ then applying $\tilde\varphi$. In other words, the following diagram commutes:}
...
\added{2}{There is a similar notion of isomorphic permutation groups, where the natural action is preserved:
    ...
    The condition on $\tau,\psi$ says that the following diagram commutes:}
...
\added{3}{In the special case that $G = H$ and $\psi = \Id_G$, then permutation isomorphism yields equivalence: for $g \in G$ and $\alpha \in \Omega$, since $G,H$ are permutation isomorphic, we have $\tau(\alpha^g) = \tau(\alpha)^{\Id_G(g)} = \tau(\alpha)^g$, so the actions are equivalent via $\tau$.}
...
\begin{example}\label{eg:rra_orbits_stabilisers}
    \begin{enumerate}[(a)]
        ...
        \item \added{1}{When $G \leq K$, we can extend $\mathcal{R}$ to a $G$-action $\mathcal{R}^K : G \to \Sym(K)$ on $K$, by $\alpha^g = \alpha g$ for $\alpha \in K$ and $g \in G$. Then the orbits are the left cosets $\alpha G$ of $G$ in $K$, so by \hyperref[thm:lagrange]{Lagrange's theorem}, there are $|K : G|$ orbits, each with size $G$.}
    \end{enumerate}
\end{example}
...
\begin{theorem}[orbit-stabiliser (OST); \added{1}{Fact 1 in \cite{blaha1992}}]\label{thm:orbit_stabiliser}
    The map $\added{2}{G_\alpha} \backslash G \to \alpha^G$ given by $\added{2}{G_\alpha}g \mapsto \alpha^g$ is a bijection. \added{1}{So, the index of $\added{2}{G_\alpha}$ in $G$ is $|G : \added{2}{G_\alpha}| = |\alpha^G|$, and $|G| = |\alpha^G| |\added{2}{G_\alpha}|$ by \hyperref[thm:lagrange]{Lagrange's theorem}.}
\end{theorem}
...
\added{2}{Recall that a \textbf{maximal subgroup} $H$ of $G$ is a subgroup $H \leq G$ such that $H < K \leq G$ implies $K = G$.

    \begin{lemma}\label{lem:conjugate_of_maximal_subgroup_is_maximal}
        If $H \leq G$ is maximal, then $H^g$ is maximal for all $g \in G$.
    \end{lemma}
...
\added{2}{If $\Delta \subseteq \Omega$ is a block, $G$ may not act on $\Delta$, as if $\Delta^g \cap \Delta = \emptyset$ for $g \in G$, then for $\alpha \in \Delta$, $\alpha^g \not\in \Delta$. But the following lemma shows that there is a subgroup of $G$ that acts on $\Delta$.
    ...
    \begin{proof}
        \begin{enumerate}[(a)]
            \item We have $\beta \in \Delta^{gh}$ if and only if $\beta = \alpha^{gh}$ with $\alpha \in \Delta$, if and only if $\beta = (\alpha^g)^h$ with $\alpha \in \Delta$ and $\alpha^g \in \Delta^g$, if and only if $\beta \in (\Delta^g)^h$.
